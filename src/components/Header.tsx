  ...
  return (
    <motion.header 
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled ? 'bg-[#0a0a0a]/80 backdrop-blur-xl border-b border-white/5 py-3' : 'bg-transparent py-6'
      }`}
      initial={{ y: -100 }}
      animate={{ y: 0 }}
      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between">
          ...
          >
            ...
            <span className="font-black text-2xl text-white tracking-wide group-hover:text-[#36e826] transition-colors duration-300 drop-shadow-[0_0_10px_rgba(54,232,38,0.3)]">
              OVERBOX
            </span>
          </div>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="lg:hidden fixed inset-0 top-[60px] bg-[#0a0a0a] z-40 overflow-y-auto"
          >
            <div className="container mx-auto px-4 py-8 flex flex-col gap-2">
              {navLinks.map((link) => (
                ...
                >
                  ...
                  {currentPage === link.id && <ChevronRight size={20} />}
                </button>
              ))}
              ...
              <a 
                ...
                target="_blank"
                rel="noreferrer"
                className="flex items-center justify-center p-4 rounded-xl bg-[#36e826] text-black font-black text-lg mt-4"
              >
                Вступить в Discord
              </a>
            </div>
